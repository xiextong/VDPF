'''Fusion module based on global and local features of self-attention and cross-attention'''
import torch
...
class Para_Att(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        x_CA = self.cross_attn(self.norm1(x)) #nor
...
class AMF(BaseModule):
    ...
    def forward(self, x_s, x_t):
        ...
        for idx in range(len(self.in_channels)):
            ...
            x_s_i_token = x_s[idx].view(-1, hwc_shape[2], hwc_shape[0]*hwc_shape[1]).permute(0, 2, 1) #2 25 768
            x_t_i_token = x_t[idx].view(-1, hwc_shape[2], hwc_shape[0]*hwc_shape[1]).permute(0, 2, 1)

            x_s_i_tmp = torch.cat((x_t_i_token, x_s_i_token), dim=1) #2 50 768
            ...
            x_s_SA = x_s_SA + x_s_i_token #残差
            x_t_SA = x_t_SA + x_t_i_token #2 256 768

            x_inter = (x_s_CA + x_t_CA).permute(0, 2, 1).view(-1, hwc_shape[2], hwc_shape[0], hwc_shape[1])
            x_s_SA = x_s_SA.permute(0, 2, 1).view(-1, hwc_shape[2], hwc_shape[0], hwc_shape[1]) #2 768 16 16
            x_t_SA = x_t_SA.permute(0, 2, 1).view(-1, hwc_shape[2], hwc_shape[0], hwc_shape[1])
            out = torch.concat((x_s_SA, x_inter, x_t_SA), dim=1) #2 768*3 16 16

            out_ = out.view(-1, self.in_channels[idx]*3, hwc_shape[0]*hwc_shape[1]).permute(0, 2, 1) #2 256 768*3
            out = getattr(self, 'mlp_' + str(idx))(out_) #mlp #2 256 768
            out = out.permute(0, 2, 1).view(-1, self.in_channels[idx], hwc_shape[0], hwc_shape[1]) #2 768 16 16
...
class Fusion(BaseModule):
    def __init__(self, dim, num_heads, out_channels=None, fusion_loss=None, lamda = 1, init_cfg=None):
        super().__init__(init_cfg)
        self.dim = dim
        self.num_heads = num_heads
        self.criterion = fusion_loss if fusion_loss is not None else nn.MSELoss(reduction='mean')
        self.lamda = lamda
        self.Fusion_Att = Fusion_Att(dim=dim, num_heads=self.num_heads)
...
x_t = [torch.randn(2,768, 16, 16),torch.randn(2,768, 16, 16),torch.randn(2,768, 16, 16)] #num B EMB H W
x_s = [torch.randn(2,768, 16, 16),torch.randn(2,768, 16, 16),torch.randn(2,768, 16, 16)]
model = AMF(in_channels=[768,768,768],num_heads=[12,12,12])
a = model(x_s,x_t)